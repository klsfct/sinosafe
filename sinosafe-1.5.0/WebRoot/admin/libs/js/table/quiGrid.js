var widthModifyFlag = 0; (function($) {
    ...
    $.quiui.controls.Grid.quiExtend($.quiui.core.UIComponent, {
        ...
        _setHeight: function(h) {
            ...
            //2016-2-25 修改矫正4像素
            var gridHeaderHeight = p.headerRowHeight * (g._columnMaxLevel - 1) + p.headerRowHeight - 1+4;
            h -= gridHeaderHeight;
            if (h > 0) {
                g.gridbody.height(h);
                if (h > 18) {
        ...
        beginEdit: function(rowParm, containerBulider) {
            ...
            l = g.columns.length; i < l; i++) {
                ...
                var editParm = {
                    record: rowdata,
                     //2016-03-04 修改获取列名方式，支持2级
                     //value: rowdata[column.name],
                    value: g._getDataByColumnName(rowdata,column.name),
                    column: column,
                    rowindex: rowdata.__index,
                    grid: g
                };
                ...
                for (var columnid in g.editors[rowdata.__id]) {
                    ...
                    if (column.name) {
                     ...
                     //newdata[column.name] = o.editor.getValue(o.input, o.editParm)
                    	var _tempValue = o.editor.getValue(o.input, o.editParm)
                    	g._setDataByColumnName(newdata,column.name,_tempValue);
                    }
                }
        ...
        updateCell: function(arg, value, rowParm) {
            ...
            //rowdata[column.name] = value;
            	g._setDataByColumnName(rowdata,column.name,value)
        ...
        getChildren: function(rowParm, deep) {
            ...
            function loadChildren(data) {
                if (data[p.tree.childrenName]) {
                    for (var i = 0,
                    l = data[p.tree.childrenName].length; i < l; i++) {
        ...
        _initBuildGridHeader: function() {
            ...
            for (var level = 1; level <= g._columnMaxLevel; level++) {
                ...
                $(columns).each(function(i, column) { (column.frozen ? trf: tr).append(g._createHeaderCell(column));
                    if (column.__leaf) {
                        var colwidth = column._width;
                        if (!column.frozen) {
        ...
        //2016-03-04 修改获取列名方式，支持2级
        _getDataByColumnName:function(rowData,columnName){
        	var aaa=eval("rowData."+columnName);
        	return eval("rowData."+columnName);
        },
        _setDataByColumnName:function(rowData,columnName,dataValue){
        	eval("rowData."+columnName+"="+dataValue);
        },
        _clearGrid: function() {
            var g = this,
            p = this.options;
        ...
        _fillGridBody: function(data, frozen) {
            ...
            if (a > 0) {
                ...
                gridhtmlarr.push("</tbody></table></div>"); (frozen ? g.f.gridbody: g.gridbody).html(gridhtmlarr.join(""));
                if (!g.enabledGroup()) {
        ...
        _getCellContent: function(rowdata, column) {
            ...
            var value = column.name ? this._getDataByColumnName(rowdata,column.name) : null;
            var g = this,
            p = this.options;
            var content = "";
            if (column.render) {
        ...
        _applyEditor: function(obj) {
            ...
            if (column.editor.type && p.editors[column.editor.type]) {
                ...
                var currentdata = g._getDataByColumnName(rowdata,columnname);
                var editParm = {
                    record: rowdata,
                    value: currentdata,
                    column: column,
        ...
        _getTotalCellContent: function(column, data) {
            ...
            if (column.totalSummary) {
                ...
                var max = parseFloat(g._getDataByColumnName(data[0],column.name));
                var min = parseFloat(g._getDataByColumnName(data[0],column.name));
                for (var i = 0; i < data.length; i++) {
                    ...
                    var value = parseFloat(g._getDataByColumnName(data[i],column.name));
                    if (!value) {
                        continue
                    }
                    sum += value;
        ...
        _setEvent: function() {
            var g = this,
            p = this.options;
            g.grid.bind("mousedown.grid",
            function(e) {
                g._onMouseDown.call(g, e)
            });
            g.grid.bind("dblclick.grid",
            function(e) {
                g._onDblClick.call(g, e)
            });
            g.grid.bind("contextmenu.grid",
            function(e) {
                return g._onContextmenu.call(g, e)
            });
            g.grid.bind("mouseup.grid",
            function(e) {
                g._onMouseUp.call(g, e)
            });
            g.grid.bind("click.grid",
            function(e) {
                g._onClick.call(g, e)
            });
            ...
            g.grid.bind("keydown.grid",
            function(e) {
                if (e.ctrlKey) {
                    g.ctrlKey = true
                }
            });
            g.grid.bind("keyup.grid",
            function(e) {
                delete g.ctrlKey
            });
            g.gridbody.bind("scroll.grid",
            function() {
    ...
    a.quiui.controls.Resizable.quiExtend(a.quiui.core.UIComponent, {
        ...
        _start: function(d) {
            ...
            a(document).bind("selectstart.resizable",
            function() {
                return false
            });
            a(document).bind("mouseup.resizable",
            function() {
                b._stop.apply(b, arguments)
            });
            a(document).bind("mousemove.resizable",
            function() {
