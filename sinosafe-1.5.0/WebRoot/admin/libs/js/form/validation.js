        ...
        if (settings.inlineValidation == true) {
            if (!settings.returnIsValid) {
                ...
                if (settings.liveEvent) {
                    ...
                    $(this).find("[class*=selectCustom]").live("boxClose",
                    function(caller) {
                        _inlinEvent(this)
                    });
                    $(this).find("[class*=suggestion]").live("validate",
                    function(caller) {
                        _inlinEvent(this)
                    });
                    $(this).find("[class*=suggestion]").live("validate2",
                    function(caller) {
                        _inlinEvent(this)
                    })
                } else {
                    ...
                    $(this).find("[class*=selectCustom]").bind("boxClose",
                    function(caller) {
                        _inlinEvent(this)
                    });
                    $(this).find("[class*=suggestion]").bind("validate",
                    function(caller) {
                        _inlinEvent(this)
                    });
                    $(this).find("[class*=suggestion]").bind("validate2",
                    function(caller) {
                        _inlinEvent(this)
                    })
                }
    ...
    $.validationEngine = {
        ...
        validateCall: function(caller, rules) {
            ...
            function _required(caller, rules) {
                if ($(caller).hasClass("validate[required]") && $(caller).hasClass("selectTree")) {
                    if ($(caller).attr("relValue") == null || $(caller).attr("relValue") == "") {
                        $.validationEngine.isError = true;
                        promptText = "* 此项为必选."
                    }
                } else {
                    if ($(caller).hasClass("validate[required]") && $(caller).hasClass("selectCustom")) {
                        if ($(caller).attr("relValue") == null || $(caller).attr("relValue") == "") {
                            $.validationEngine.isError = true;
                            promptText = "* 此项为必选."
                        }
                    } else {
                        if ($(caller).hasClass("validate[required]") && $(caller).hasClass("suggestion")) {
                            if ($(caller).attr("relText") == null || $(caller).attr("relText") == "") {
                                $.validationEngine.isError = true;
                                promptText = "* 此项为必选."
                            }
                        } else {
                            if ($(caller).hasClass("validate[required]") && $(caller).hasClass("fileComponent")) {
                                ...
                                if ($(caller).is("select")) {
                                    ...
                                    if ($(caller).hasClass("validate[required]") && $(caller).hasClass("filter")) {
                                        if ($(caller).attr("relValue") == null || $(caller).attr("relValue") == "") {
                                            $.validationEngine.isError = true;
                                            promptText = "* 此项为必选."
                                        }
                                    } else {
                                        if ($(caller).hasClass("validate[required]") && $(caller).hasClass("lister")) {
                                            if ($(caller).attr("relValue") == null || $(caller).attr("relValue") == "") {
                                                $.validationEngine.isError = true;
                                                promptText = "* 此项为必选."
                                            }
                                        } else {
                                            if ($(caller).hasClass("validate[required]") && $(caller).hasClass("listerTree")) {
                                                if ($(caller).attr("relValue") == null || $(caller).attr("relValue") == "") {
                                                    $.validationEngine.isError = true;
                                                    promptText = "* 此项为必选."
                                                }
                                            } else {
                                                if ($(caller).is("textarea")) {
                                                    if ($(caller).val() == null || $(caller).val() == "") {
                                                        $.validationEngine.isError = true;
                                                        promptText = "* 此项为必选."
                                                    }
