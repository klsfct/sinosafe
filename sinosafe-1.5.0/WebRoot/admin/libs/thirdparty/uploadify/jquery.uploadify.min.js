...
SWFUpload.Console.writeLine = function(g) {
    ...
    var a = {
        onDialogOpen: function() {
            var d = this.settings;
            this.queueData.errorMsg = "上传文件未成功，出现异常:";
        ...
        onSelect: function(h) {
            ...
            for (var g in this.queueData.files) {
                ...
                if (f.uploaded != true && f.name == h.name) {
                    var e = confirm('文件命名 "' + h.name + '" 已经在上传列表中。\n你想替换现有的上传列表项?');
                    if (!e) {
            ...
            if (c.inArray("onSelectError", e.overrideEvents) < 0) {
                ...
                case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
                    if (e.queueSizeLimit > f) {
                        this.queueData.errorMsg += "\n上传的文件数量已经超出系统限制的 " + f + "个文件。";
                    } else {
                        this.queueData.errorMsg += "\n上传的文件数量已经超出单次选择限制的" + e.queueSizeLimit + "个文件。";
                    }
                    break;
                case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                    this.queueData.errorMsg += '\n选择的文件[ "' + d.name + '"] 大小超过系统限制 (' + e.fileSizeLimit + ")。";
                    break;
                case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
                    this.queueData.errorMsg += '\n选择的文件[ "' + d.name + '"] 不存在。';
                    break;
                case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                    this.queueData.errorMsg += '\n选择的文件[ "' + d.name + '"] 是不被接受的文件类型 (' + e.fileTypeDesc + ")。";
                    break;
                }
            }
        ...
        onUploadStart: function(d) {
            ...
            if (e.checkExisting) {
                c.ajax({
                    ...
                    success: function(h) {
                        if (h == 1) {
                            var g = confirm('这个文件[ "' + d.name + '"] 已经存在在服务器上。\n你想替换现有的文件?');
                            if (!g) {
