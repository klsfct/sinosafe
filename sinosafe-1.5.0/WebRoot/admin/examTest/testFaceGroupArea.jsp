...
<body>
	<s:form id="paramFormId" name="paramForm" method="post"
		action="groupFaceTestByArea" namespace="/admin/examTest">
				...
				<s:iterator value="resultList" status="st1">
					<tr>
						...
						<td><s:if test="#st1.isOdd()">
								<div
									style="width:<s:property value="resultList[#st1.index][3]" />;height:20px;border-style:solid;border-width:1px;border-color:black;background-color:#7cb5ec"></div>
							</s:if> <s:else>
								<div
									style="width:<s:property value="resultList[#st1.index][3]" />;height:20px;border-style:solid;border-width:1px;border-color:black;background-color:#90ed7d"></div>
							</s:else></td>
					</tr>
				</s:iterator>
			</table>
