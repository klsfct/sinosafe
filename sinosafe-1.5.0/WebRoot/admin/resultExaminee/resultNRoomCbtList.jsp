...
<body>
		...
		<table class="tableStyle" mode="list">
			...
			<s:iterator value="resultList" status="st1">
				<tr>
					...
					<td>
						<s:property value="resultList[#st1.index][1]" />
					</td>
					<td><s:property value="resultList[#st1.index][2]" /></td>
					<td><a
						href="javascript:findNResultExamineeForRpt(<s:property value="resultList[#st1.index][0]" />);"><span class="icon_print">打印报表</span></a><%-- <a
						href="javascript:findNResultExamineeForModify(<s:property value="resultList[#st1.index][0]" />);"><span class="icon_ok">处理上报</span></a> --%></td>
				</tr>
			</s:iterator>
		</table>
	</div>
	<script type="text/javascript">
		...
		//打印
		function findNResultExamineeForRpt(roomCbtId){
			//打开窗口
			var diag = new top.Dialog();
			diag.Title = "核查表打印";
			diag.URL = "../resultExaminee/findNResultExamineeCbtForRpt.action?roomCbtId="+roomCbtId+"&examPlaceVo.examPlace="+$("#examPlaceVo_examPlace").val()+"&subject="+$("#subjectId").val();
			diag.Width = 950;
			diag.Height = 540;
		...
		//打印
		function findNResultExamineeForModify(roomCbtId){
			//打开窗口
			var diag = new top.Dialog();
			diag.Title = "核查结果上报";
			diag.URL = "../resultExaminee/findNResultExamineeCbtForModify.action?roomCbtId="+roomCbtId+"&examPlaceVo.examPlace="+$("#examPlaceVo_examPlace").val()+"&subject="+$("#subjectId").val();
			diag.Width = 950;
			diag.Width = 950;
			diag.Height = 540;
			diag.ShowButtonRow = false;
