...
<body>
	<s:form id="paramFormId" name="paramForm" method="post"
		...
		<s:if test="subjectList !=null">
			<span> 选择场次：</span>
			<s:select list="subjectList" name="subjectVo.id" id="subjectId"
		...
		<table class="tableStyle" mode="list">
			...
			<s:iterator value="resultList" status="st1">
				<tr>
					...
					<td><a
						href="javascript:findQResultExamineeForRpt(<s:property value="resultList[#st1.index][0]" />);"><span class="icon_print">打印报表</span></a>
						<%-- <a
						href="javascript:findQResultExamineeForModify(<s:property value="resultList[#st1.index][0]" />);"><span class="icon_ok">处理上报</span></a> --%>
					</td>
				</tr>
			</s:iterator>
		</table>
	...
	<script type="text/javascript">
		...
		//打印
		function findQResultExamineeForRpt(roomCbtId){
			//打开窗口
			var diag = new top.Dialog();
			diag.Title = "核查表打印";
			diag.URL = "../resultExaminee/findQResultExamineeCbtForRpt.action?roomCbtId="+roomCbtId+"&examPlaceVo.examPlace="+$("#examPlaceVo_examPlace").val()+"&subject="+$("#subjectId").val();
			diag.Width = 950;
			diag.Height = 540;
		...
		//打印
		function findQResultExamineeForModify(roomCbtId){
			//打开窗口
			var diag = new top.Dialog();
			diag.Title = "核查结果上报";
			diag.URL = "../resultExaminee/findQResultExamineeCbtForModify.action?roomCbtId="+roomCbtId+"&examPlaceVo.examPlace="+$("#examPlaceVo_examPlace").val()+"&subject="+$("#subjectId").val();
			diag.Width = 950;
			diag.Height = 540;
			diag.ShowButtonRow = false;
			diag.show();
