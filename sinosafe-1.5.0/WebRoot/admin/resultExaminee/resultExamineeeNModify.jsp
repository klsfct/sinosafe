...
<body>
	<s:form id="paramFormId" name="paramForm" method="post"
		action="modifyNResultExaminee" namespace="/admin/resultExaminee">
		...
		<s:hidden id="reportSumId" value="%{resultExamineeVoList.size}"></s:hidden>
		<div class="padding_right5 padding_top5">
			<span style="color:red;font-size:18px">1、默认处理结果是“核查无误”，如有违纪人员请修改处理结果。</span><br />
			<span style="color:red;font-size:18px">2、鼠标挪至“姓名”可预览照片。</span>
		</div>
		<div class="padding_right5 padding_top5">
			<span style="color:red;font-size:18px">3、 《<s:if
					test="resultExamineeVoList[0].subject==0">
			测试卷
		 ...
		 <s:elseif test="subject==6">
			第六场
				...
				<s:if
					test="null!=resultExamineeVoList && resultExamineeVoList.size()>0">
					<s:property value="resultExamineeVoList[0].examPlaceName" />》
						</s:if>
				<s:property value="beginRoom" />&nbsp;至&nbsp;<s:property
					value="endRoom" />考场
			</span>
			...
			<table class="tableStyle" mode="list">
				...
				<s:iterator value="resultExamineeVoList" status="st1">
					<tr>
						...
						<td class="ali02"><strong><s:property
									value="examRoom" /></strong></td>
						<td class="ali02"><strong><s:property
									value="seatNumber" /></strong></td>
						...
						<td><s:select
								...
								list="verifyResultList"></s:select> <s:hidden
								name="resultExamineeVoList[%{#st1.index}].licence"
								value="%{licence}"></s:hidden></td>
						<td><s:if test="isVerify==true">
								<span class="icon_ok" style="color:green">已上报</span>
							</s:if> <s:else>
								未上报
							</s:else></td>
						<td><s:date name="createDate" format="yyyy-MM-dd HH:mm" /></td>
						<td><s:if test="null==verifyDate"></s:if> <s:date
								name="verifyDate" format="yyyy-MM-dd HH:mm" /></td>
					</tr>
				</s:iterator>
			</table>
		</div>
	...
	<div id="mydiv1"
		style="width:320px;height:180px;position:absolute;display:none;">
		<table>
			<tr>
				<td><img id="photo1Id" src="" style="width:132px;" /></td>
				<td><img id="photo2Id" src="" style="width:132px;" /></td>
			</tr>
		</table>
	</div>
	<script type="text/javascript">
		function submitData() {
			if ($("#reportSumId").val() == 0) {
				alert("没有批量上报的疑似人员数据！");
			} else {
				if (confirm("确认填写信息无误，提交上报疑似人员处理信息？")) {
					$("#paramFormId").submit();
				} else {
					return false;
				}
