...
import com.sinotn.examsafety.dao.SocketExaminee6DAO;
...
public class ExamineeData6ReceiveImpl implements IFaceDataReceiveService {
	private static Logger logger = Logger.getLogger(ExamineeData6ReceiveImpl.class);
	private ResultExaminee6DAO resultExaminee6DAO = null;

	private SocketExaminee6DAO socketExaminee6DAO = null;
	...
	private final static String SUBJECT = "6";

	@Override
	public void executeDataReceive() {
		// 从socketExaminee6中取出待处理数据
		List<SocketExaminee6> socketList = this.getSocketExaminee6DAO()
				.findSocketExaminee6List(recordNumber);
		if (socketList != null && !socketList.isEmpty()) {
			for (SocketExaminee6 socketExaminee : socketList) {
				...
				if (socketExaminee.getIsException()) {// 退场数据处理
					...
					// 进场数据处理
					try {
						ResultExaminee6 resultExaminee = this
								.getResultExaminee6DAO().getResultExaminee(licence);// 场次一考生对象
						...
						if (resultExaminee != null) {
							...
							if (null != examPlace) {
								...
								logger.error("数据考点不存在——异常ID：【Table：SOCKET_EXAMINEE_6  ID:" + socketExaminee.getId() + "】");
							}
							...
							logger.error("数据异常——异常ID：【Table：SOCKET_EXAMINEE_6  ID:" + socketExaminee.getId() + "】");
						}
					} catch (Exception e) {
						logger.error("无效数据——异常ID：【Table：SOCKET_EXAMINEE_6  ID:" + socketExaminee.getId() + "】");
					}
	...
	private ResultExaminee6 getResultExaminee(ResultExaminee6 resultExaminee,
			SocketExaminee6 socketExaminee) {
	...
	public SocketExaminee6DAO getSocketExaminee6DAO() {
		return socketExaminee6DAO;
	}



	public void setSocketExaminee6DAO(SocketExaminee6DAO socketExaminee6DAO) {
		this.socketExaminee6DAO = socketExaminee6DAO;
	}
