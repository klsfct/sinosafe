...
import com.sinotn.examsafety.dao.SocketExaminee2DAO;
...
public class ExamineeData2ReceiveImpl implements IFaceDataReceiveService {
	private static Logger logger = Logger.getLogger(ExamineeData2ReceiveImpl.class);
	private ResultExaminee2DAO resultExaminee2DAO = null;

	private SocketExaminee2DAO socketExaminee2DAO = null;
	...
	public void executeDataReceive() {
		...
		List<SocketExaminee2> socketList = this.getSocketExaminee2DAO()
				.findSocketExaminee2List(recordNumber);
		if (socketList != null && !socketList.isEmpty()) {
			for (SocketExaminee2 socketExaminee : socketList) {
				...
				if (socketExaminee.getIsException()) {// 退场数据处理
					...
					// 进场数据处理
					try {
						ResultExaminee2 resultExaminee = this
								.getResultExaminee2DAO().getResultExaminee(licence);// 场次一考生对象
						if (resultExaminee != null) {
							...
							if (null != examPlace) {
								...
								logger.error("数据考点不存在——异常ID：【Table：SOCKET_EXAMINEE_2  ID:" + socketExaminee.getId() + "】");
							}
							...
							logger.error("数据异常——异常ID：【Table：SOCKET_EXAMINEE_2  ID:" + socketExaminee.getId() + "】");
						}
					} catch (Exception e) {
						logger.error("无效数据——异常ID：【Table：SOCKET_EXAMINEE_2  ID:" + socketExaminee.getId() + "】");
					}
	...
	private ResultExaminee2 getResultExaminee(ResultExaminee2 resultExaminee,
			SocketExaminee2 socketExaminee) {
	...
	public SocketExaminee2DAO getSocketExaminee2DAO() {
		return socketExaminee2DAO;
	}

	public void setSocketExaminee2DAO(SocketExaminee2DAO socketExaminee2DAO) {
		this.socketExaminee2DAO = socketExaminee2DAO;
	}
