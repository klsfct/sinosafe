...
public class ResultCard2DAO extends BaseDAO<ResultCard2, Long> {
	 ...
	 * @return
	 */
	public ResultCard2 getResultCard(Long examPlace,Integer examRoom){
		String hql = "from ResultCard2 t where t.examPlace = ? and t.examRoom = ?";
		Session session = this.getSessionFactory().getCurrentSession();
		Query query = session.createQuery(hql);
		query.setLong(0, examPlace);
		query.setInteger(1, examRoom);
		@SuppressWarnings("unchecked")
		List<ResultCard2> list = query.list();
		if(list.isEmpty() ){
			return null;
		}
		else{
	...
	public long findCardsByRoom(String subject,long examPlaceId,int examRoom){
		String hql = "select count(*) from ResultCard2 t where t.subject = ? and t.examPlace = ? and t.examRoom = ?";
		Session session = this.getSessionFactory().getCurrentSession();
		Query query = session.createQuery(hql);
